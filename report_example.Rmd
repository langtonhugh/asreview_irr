...
first_dist_df <- first_df %>% 
  filter(.data[[relevance_1]] == 1) %>% 
  mutate(missed_flag = if_else(first_relevant_vec %in% secon_dist_vec,
                               "missed",
                               "not missed"),
...
secon_dist_df <- secon_df %>% 
  filter(.data[[relevance_2]] == 1) %>% 
  mutate(missed_flag = if_else(secon_relevant_vec %in% first_dist_vec,
                          "missed",
                          "not missed"),
         missed_flag = as.factor(missed_flag))
...
If there are no green squares for relevant studies, then less than 0.5% of total abstracts were rated as relevant. Due to rounding, these studies won't show in the graphic.
