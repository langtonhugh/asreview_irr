...
error = function(e) {
  warning("Unable to read the csv. Check if your file is a valid csv.")
  stop(e)
}) 
...
if(nrow(first_df) != nrow(secon_df)) {
  stop("Size of the compared dataframes does not match. Check for issues in the loaded files or ")
}


if ('record_id' %in% names(first_df) & 'record_id' %in% names(secon_df)) {
  first_record_id <- first_df$record_id
  secon_record_id <- secon_df$record_id
} else {
   stop("Cannot find a column named 'record_id'. If you have a column that contains the ID of a record, rename ")
}
