...
# Comparison of different Kappa methods
...
Regular old kappa (with NA's coded):
```{r}
kappa2(na_as_cat)
```
...
Gwent's kappa is the following (NA's as category):
```{r}
 gwet.ac1.raw(na_as_cat)

```
Gwent's kappa on complete cases:
```{r}
gwet.ac1.raw(complete_cases)
```
Krippendorf's alpha is the following:
```{r}
